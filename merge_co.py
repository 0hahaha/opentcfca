...
def build_index(entries):
    ...
    for i, entry in enumerate(entries):
        text = remove_non_french_characters(entry["text"])
...
def merge_json_files(file_list, threshold=0.9):
    ...
    for i, entry1 in enumerate(all_entries):
        ...
        words = [get_words(entry1["text"], 0, 2), get_words(entry1["text"], -3, -1), get_words(entry1["text"], -7, -5), get_words(entry1["text"], 10, 12)]
        for word in words:
